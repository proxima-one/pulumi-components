...
export interface DoClusterOutputs {
	...
	constructor(name: string, args: DoClusterInputs, opts?: pulumi.ComponentResourceOptions) {
		...
		const cluster = new digitalocean.KubernetesCluster(name, {
			...
			nodePool: {
				...
				maxNodes: args.nodePools[0].autoScale ? args.nodePools[0].autoScale.minNodes : undefined,
				minNodes: args.nodePools[0].autoScale ? args.nodePools[0].autoScale.minNodes : undefined,
		...
		for (let i = 1; i < args.nodePools.length; i++) {
			const storageNodePool = new digitalocean.KubernetesNodePool(args.nodePools[i].name, {
				...
				maxNodes: args.nodePools[i].autoScale ? args.nodePools[i].autoScale?.minNodes : undefined,
				minNodes: args.nodePools[i].autoScale ? args.nodePools[i].autoScale?.minNodes : undefined,
