...
export class WebServiceDeployer extends KubernetesServiceDeployer {
  public deploy(app: WebService): DeployedServiceApp {
    ...
    for (const [partName, part] of _.entries(app.parts)) {
      ...
      const pvcs = pulumi.output(part.pvcs).apply(
        (pvcs) =>
          pvcs?.map((pvcRequest) =>
      ...
      const volumes: pulumi.Input<
        pulumi.Input<k8s.types.input.core.v1.Volume>[]
      > = pvcs.apply((pvcArr) =>
        pvcArr
          .map(
            (pvcData) =>
