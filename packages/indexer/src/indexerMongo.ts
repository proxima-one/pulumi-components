...
export class IndexerMongo extends proxima.MongoDB {
  public constructor(name: string, args: IndexerMongoArgs, opts: pulumi.ComponentResourceOptions) {
    super(name, {
      ...
      mongoExpress: {
        endpoint: pulumi.output(args.publicHost).apply(host => `${name}-mongo-express.${host}`)
      }
    }, opts);
  }
}
