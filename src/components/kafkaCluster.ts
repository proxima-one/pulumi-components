...
export class KafkaCluster extends pulumi.ComponentResource {
  ...
  public constructor(
    ...
    this.kafka = new k8s.apiextensions.CustomResource(name, {
    ...
    if (args.kafka.listeners?.external) {
      this.kafkaUser = new k8s.apiextensions.CustomResource(`${name}-ext-user`, {
        apiVersion: "kafka.strimzi.io/v1beta2",
        kind: "KafkaUser",
        metadata: {
          namespace: args.namespace.metadata.name,
...
async function waitUntilSucceed<T>(resource: string, func: () => Promise<T>, timeout?: number, interval: number = 500): Promise<T> {
  ...
  while (true) {
    try {
      ...
      if (Date.now() > startTimestamp + timeout)
        throw new Error(`Error waiting for ${resource}: timeout of ${timeout}ms exceeded`);
    }
  }
}
